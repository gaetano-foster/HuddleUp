...
ConstructDisplay(
	...
	if (!(display->win = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, 0))) {
		fprintf(stderr, "Couldn't initialize display.\n%s", SDL_GetError());
		return 0;
	}

	if (!(display->ren = SDL_CreateRenderer(display->win, -1, 0))) {
		fprintf(stderr, "Couldn't initialize display.\n%s", SDL_GetError());
		return 0;
	}
...
RenderPlane(
	...
	for (int y = 1; y < SCR_H / 2; y++) {
		...
		for (int x = 1; x < SCR_W; x++) {
			...
			SDL_Color rgb;
			int tx = sample_x * plane.tex_width;
			int ty = sample_y * plane.tex_height;
...
Render(
	...
	RenderPlane(display->ren, state->floor, state->camera.x, state->camera.y, state->camera.a, 3.14f / 1.5f);
